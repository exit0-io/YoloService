...
class TestGetPredictedImage(unittest.TestCase):
    ...
    def test_get_predicted_image(self):
        """Test GET /image/predicted/{uid}"""
        uid = self.create_prediction()
        response = client.get(f"/image/predicted/{uid}")
        self.assertEqual(response.status_code, 200)
        self.assertIn(response.headers["content-type"], ["image/jpeg", "image/jpg"])
    
    def test_get_predicted_image_not_found(self):
        """Test GET /image/predicted/{uid} with non-existent UID"""
        response = client.get("/image/predicted/non-existent-uid")
