...
class TestGetOriginalImage(unittest.TestCase):
    ...
    def test_get_original_image(self):
        """Test GET /image/original/{uid}"""
        uid = self.create_prediction()
        response = client.get(f"/image/original/{uid}")
        self.assertEqual(response.status_code, 200)
        self.assertIn(response.headers["content-type"], ["image/jpeg", "image/jpg"])
    
    def test_get_original_image_not_found(self):
        """Test GET /image/original/{uid} with non-existent UID"""
        response = client.get("/image/original/non-existent-uid")
